  ...
  if (structure == 'blocked'){
    if (trialType =='ab') {
      for(let i = 0;i<15;i++){
        ...
        if (threeIndex == 1){
          genLeft.push(aList[i])
          genCorrect.push(aList[i])
          genMid.push(aList[incorrectArr])
          genRight.push(aList[wrongA])
        } else if (threeIndex == 2){
          genRight.push(aList[i])
          genCorrect.push(aList[i])
          genLeft.push(aList[incorrectArr])
          genMid.push(aList[wrongA])
        } else {
          genMid.push(aList[i])
          genCorrect.push(aList[i])
          genRight.push(aList[incorrectArr])
    ...
    if (trialType == 'ab' || trialType == 'bc' ) {
      ...
      for(let i = 0;i<15;i++){ 
        ...
        while (i == wrongA || incorrectArr == wrongA) {
          wrongA = Math.floor(Math.random() * 14)
        } 
        threeIndex = Math.floor(Math.random() * 3)+1
        if (threeIndex == 1){
